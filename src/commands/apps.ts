...
import { deepMerge, flatToNested, isPlainObject, parseAssignments } from "../lib/dotted";
...
export function buildAppBody(
  ...
  for (const [k, v] of Object.entries(parseAssignments(arraySets))) flat[unshorten(k)] = v.split(",").map((s) => s.trim());
  const base = isPlainObject(body) ? body : {};
  const merged = deepMerge(base, flatToNested(flat));
  if (name && SIGNON_DEFAULTS[name]) signonmode = SIGNON_DEFAULTS[name];
  if (name !== undefined) merged.name = name;
  if (label !== undefined) merged.label = label;
  if (signonmode !== undefined) merged.signOnMode = signonmode;
  return merged;
}
...
export function registerApps(program: Command, ctx: Ctx): Command {
  ...
  addVerbose(g.command("add").description("Add a new application. EXAMPLE: okta-cli apps add -n bookmark -l my_bookmark -s sa.url=http://my.url")
    ...
    .option("-b, --body <json>", "JSON body; FILE:<path> reads a file")
    .option("-s, --set <k=v>", "Set app parameter; prefix shortcuts sa=settings.app, v=visibility, f=features, c=credentials", collect, [])
    .option("-S, --array-set <k=v1,v2>", "Set an array app parameter (comma-separated values); same prefix shortcuts as -s", collect, []))
    .action(action(ctx, (client, opts) => client.json("POST", "/apps", { body: buildAppBody(opts.name, opts.signonmode, opts.label, opts.set, parseBody(opts.body), opts.arraySet) })));
