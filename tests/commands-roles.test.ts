...
describe("roles replace", () => {
  test("replace sends only label and description (Okta rejects id/created/lastUpdated/isCloneable/permissions)", async () => {
    srv = startServer([
    ...
    expect(srv.calls.length).toBe(2); // GET existing role + PUT
    expect(srv.calls.at(-1)!.method).toBe("PUT");
    expect(srv.calls.at(-1)!.body).toEqual({ label: "Renamed", description: "d" });
  });
});
