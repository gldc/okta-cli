...
import { formatResult, toCsv, toSortedJson, toTable, toYaml } from "../src/lib/output";
...
describe("output", () => {
  ...
  test("toCsv flattens dotted keys sorted, excel dialect", () => {
    ...
    // unix dialect mirrors Python's csv QUOTE_ALL: every field is quoted.
    expect(toCsv(rows[0], "unix")).toBe('"id","profile.firstName","profile.login","status"\n"1","Bob","bob","ACTIVE"\n');
    expect(toCsv([{ a: [1, 2] }])).toBe('a\r\n"[1,2]"\r\n');
  });
  test("toYaml sorts keys, matching Python's yaml.safe_dump(sort_keys=True)", () => {
    expect(toYaml({ b: 1, a: { d: 1, c: 2 } })).toBe("a:\n  c: 2\n  d: 1\nb: 1\n");
  });
